...
| tag                  | string                       | "img"   | No       | The element that will receive the image. Another option might be to set this to "div". Any tag besides "img" will result in the image being loaded as the css background-image for that tag. If you use TypeScript, setting the tag element changes the list of additional props you can add to the Image component. For example, if you set this to img, you can set a width and height property. If you set this to "div" then those properties do not exist for a div element |
