...
| infiniteLoop | ['off', 'auto', 'manual'] | 'off' | No | Setting to auto or manual allows slides to appear in an infinite loop.  The whole point of Pure React Carousel is to not get in the way of the DOM and CSS requirements for your brand. However, if you use "auto" mode, we require that only Slide components are allowed to be children of Slider.  That is the cost of using auto mode.  Auto mode will automatically "clone" Slide components and place them before and after your existing slides. Any non-Slide child component of Slider will cause an error to be thrown.  If, for whatever reason, you must have another component besides a Slide component as a direct child of Slider, you can use "manual" mode. In maual mode you are then responsible for providing x duplicate slides before and x duplicate slides after your "real" slides where x is equal to visibleSlides for a total of 2x slides. See manual demo [TODO: demo]
